...
# aws cp commands in shell look like aws s3 cp /media/checkpoint/audiolm-pytorch-training/sbatch_job_1.sh s3://s-laion/itsleonwu/slurm-job-1-audiolm-0.1.0/sbatch_job.sh --profile laion-stability-my-s3-bucket
...
with open(f"/media/checkpoint/audiolm-pytorch-results/{job_id}-description.txt", "w") as f:
    f.write(description)

# Read output log
with open(f"/media/checkpoint/audiolm-pytorch-results/output-{job_id}.log", "r") as f:
    output_log = f.read()
...
subprocess.run(["aws", "s3", "cp", f"/media/checkpoint/audiolm-pytorch-training/sbatch_job_{job_id}.sh", f"s3://{bucket_prefix}/{s3_folder}/sbatch_job.sh", "--profile", "laion-stability-my-s3-bucket"])
subprocess.run(["aws", "s3", "cp", f"/media/checkpoint/audiolm-pytorch-training/audiolm_pytorch_demo_laion_{job_id}.py", f"s3://{bucket_prefix}/{s3_folder}/audiolm_pytorch_demo_laion.py", "--profile", "laion-stability-my-s3-bucket"])
subprocess.run(["aws", "s3", "cp", f"/media/checkpoint/audiolm-pytorch-results/output-{job_id}.log", f"s3://{bucket_prefix}/{s3_folder}/output-{job_id}.log", "--profile", "laion-stability-my-s3-bucket"])
subprocess.run(["aws", "s3", "cp", f"/media/checkpoint/audiolm-pytorch-results/error-{job_id}.log", f"s3://{bucket_prefix}/{s3_folder}/error-{job_id}.log", "--profile", "laion-stability-my-s3-bucket"])
# output_filename = f"out_job_id_{job_id}_step_{step}.wav" # should match what is saved in audiolm_pytorch_demo_laion.py
# subprocess.run(["aws", "s3", "cp", f"/media/checkpoint/audiolm-pytorch-results/{output_filename}", f"s3://{bucket_prefix}/{s3_folder}/{output_filename}", "--profile", "laion-stability-my-s3-bucket"])


# Transfer checkpoints
for folder in ["semantic_results", "coarse_results", "fine_results"]:
    full_folder_name = f"{folder}_{job_id}"
    folder_path = f"/media/checkpoint/audiolm-pytorch-results/{full_folder_name}"
